...
@interface AZViewController () <UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate>
...
- (void)viewDidLoad
{
    ...
    for (int i = 0; i < 5000; i++) {
        ...
        AZStudent *anyStudent = [[AZStudent alloc] init];
...
- (void) sortByMonthInBackground {
    ...
    __weak AZViewController *wiakSelf = self;
...
- (NSMutableArray *) sortStudentByBirthDayMonth : (NSMutableArray *) students {
    ...
    [students sortUsingComparator:^NSComparisonResult(AZStudent *obj1, AZStudent *obj2) {
...
- (NSMutableArray *) generateSectionFromBirthDayMonth: (NSMutableArray *) array{
    ...
    for(AZSection *anySection in sectionArray) {
...
- (NSArray *)sectionIndexTitlesForTableView:(UITableView *)tableView {
    ...
    for(AZSection *anySection in self.sectionHeap) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    AZSection *anySection = [self.sectionHeap objectAtIndex:section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    AZSection *anySection = [self.sectionHeap objectAtIndex:indexPath.section];
    ...
    if([self.searchBar.text length]){
        ...
        AZStudent *anyStudent = [anySection.sectionObjectsArray objectAtIndex:indexPath.row];
...
- (IBAction)actionValueChangeSigmentedControl:(UISegmentedControl *)sender {
    ...
    switch (sender.selectedSegmentIndex) {
        ...
        case AZSortDescrptionFirstName:
        {
            [self.sortOperation cancel];
...
- (void)searchBarCancelButtonClicked:(UISearchBar *)searchBar {
    ...
    self.studentHeap = [self sortStudentByBirthDayMonth:self.studentHeap];
    self.sectionHeap = [self generateSectionFromBirthDayMonth:self.studentHeap];
    self.segmentedControl.selectedSegmentIndex = AZSortDescrptionBirthDay;
    [self clearStudentSearchRange];
    self.searchBar.text = @"";
    [self.tableView reloadData];
}
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    if([searchText length] == 1) {
        ...
        AZSection *anySection = [[AZSection alloc] init];
        anySection.sectionName = searchText;

        for(AZStudent *anyStudent in tempStudentArray){
        ...
        [tempSectionArray removeAllObjects];
        AZSection *searchSection = [self.sectionHeap firstObject];
        searchSection.sectionName = searchText;
        NSMutableArray *tempStudentArray = [[NSMutableArray alloc] init];
        
        for(AZStudent *anyStudent in searchSection.sectionObjectsArray){
...
- (void) clearStudentSearchRange {
    ...
    for(AZStudent *anyStudent in self.studentHeap) {
