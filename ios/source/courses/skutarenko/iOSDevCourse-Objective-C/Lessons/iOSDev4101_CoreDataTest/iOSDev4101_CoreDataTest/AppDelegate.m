...
#pragma mark - Addional Core Data
...
- (void)addManagedObject {
  ...
  //object settings
  [student setValue:@"FIRST_NAME" forKey:@"firstName"];
  [student setValue:@"LAST_NAME" forKey:@"lastName"];
  [student setValue:[NSDate dateWithTimeIntervalSince1970:0] forKey:@"dateOfBirth"];
  [student setValue:@3.85f forKey:@"score"];
}
...
- (void)databasePrint {
  ...
  NSArray *resultArray = [self.managedObjectContext executeFetchRequest:fetchRequest error:&fetchError];
  if (fetchError) {
  ...
  for (OLTVStudent *student in resultArray) {
