...
- (void)levelSuperman {
    NSString *originalText = [self baseText];
//    NSString *originalText = @"Agdsdg::::::         asdasd          12312adasdasd12312312     ASDASDASD;:::///....asd..a.sd.123123@@@@@@@ffff       asd  22";
    ...
    for (NSString *string in array) {
        if (string.length <= 1) {
            if ([string isEqualToString:[string uppercaseString]]) {
                tempString = [string lowercaseString];
            } else {
                tempString = [string uppercaseString];
            }
            [resultArray addObject:tempString];
//            NSLog(@"prev %@", string);
//            NSLog(@"past %@", tempString);
            continue;
        }
        //Fisrt char edit
        NSUInteger firstCharLocation = tempString.length - tempString.length;
        NSRange firstCharRange = NSMakeRange(firstCharLocation, 1);
        NSString *firstCharString = [string substringWithRange:firstCharRange];
        tempString = [string stringByReplacingCharactersInRange:firstCharRange withString:[firstCharString lowercaseString]];
        
        //Last char edit
        NSUInteger lastCharLocation = tempString.length - 1;
        NSRange lastCharRange = NSMakeRange(lastCharLocation, 1);
        int flag = 0;
        do {
            if ([tempString rangeOfCharacterFromSet:punctuationSet options:NSCaseInsensitiveSearch range:lastCharRange].location == NSNotFound) {
                NSString *lastCharString = [tempString substringWithRange:lastCharRange];
                tempString = [tempString stringByReplacingCharactersInRange:lastCharRange withString:[lastCharString uppercaseString]];
                flag = 1;
            } else {
                lastCharLocation -= 1;
                lastCharRange = NSMakeRange(lastCharLocation, 1);
//                NSLog(@"%@", tempString);
            }
        } while (flag == 0);
...
- (void)levelMaster {
    ...
    for (NSString *string in mArray) {
        ...
        NSString *tempString = @"";
        for (NSUInteger i = [array count] - 1; i > 0; i -= 1) {
            tempString = [tempString stringByAppendingString:array[i]];
            
            //Убирает пробел в последнем слове
            if (i == 1) {
                continue;
            } else {
        ...
        [inverseArray addObject:tempString];
    }
...
- (void)firstPart {
    NSString *string = [self baseText];
    NSLog(@"First part %@", [string substringToIndex:[string length] / 2]);
    NSLog(@"Second part %@", [string substringFromIndex:[string length] / 2]);
}
