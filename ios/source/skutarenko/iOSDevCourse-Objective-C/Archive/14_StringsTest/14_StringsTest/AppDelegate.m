...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    */
    ...
    NSRange range = [string rangeOfString:@"World" options:NSCaseInsensitiveSearch | NSBackwardsSearch]; //убирает чувствительность к регистру.
...
//    text = [text substringToIndex:10]; //Обрезает текст до 10-го индекса
//    text = [text substringFromIndex:10]; //После 10
//    text = [text substringWithRange:NSMakeRange(10, 10)]; // c 10-го, 10-ть след.

    /*
    NSRange range = [text rangeOfString:@"The Application Kit also uses"];
    ...
    */
    ...
    //Замена строки на строку в тексте!
    //text = [text stringByReplacingOccurrencesOfString:@"NSString" withString:@"TRA-LA-LA"];
    
//    text = [text uppercaseString]; //Все большими буквами
//    text = [text lowercaseString]; //Все маленькими буквами
//    text = [text capitalizedString]; //Все слова с большой буквы
...
//    NSLog(@"%@", text);
    ...
    */
    ...
    NSString *s3 = [s1 stringByAppendingString:s2]; //Соединение строк!
    
    NSLog(@"%@+%@ = %@ byAppendingString", s1, s2, s3);
